...
\item[\Macro{TCP\_KEEPALIVE\_INTERVAL}]
  ...
  The default keepalive interval is 300 (5 minutes); this value was chosen
...
\item[\Macro{JOB\_EXECDIR\_PERMISSIONS}]
  ...
  directory is group-accessible, permissions of \emph{0750}; likewise if set
  to \Expr{world} the directory is created with permissions \emph{0755}.
...
\item[\Macro{DAGMAN\_DEFAULT\_NODE\_LOG}]
  ...
  of a DAG.
  Note that even if \MacroNI{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} is set to
  ...
  As of version 8.1.3, three special macros are available for use in
  the \MacroNI{DAGMAN\_DEFAULT\_NODE\_LOG} specification (
  \Bold{note the "@" instead of "\$"}):
  \begin{itemize}
  \item \Expr{@(DAG\_DIR)}: The directory in which the primary DAG file
  resides.
  \item \Expr{@(DAG\_FILE)}: The name of the primary DAG file (not
  including any directory information).
  \item \Expr{@(CLUSTER)}: The cluster of this DAGMan process.
  \end{itemize}
  Note that if more than one DAG file is specified to \Condor{submit\_dag},
  the primary DAG file is whichever one comes first on the command line.

  If not defined, \MacroNI{DAGMAN\_DEFAULT\_NODE\_LOG} defaults
  to \Expr{@(DAG\_DIR)/@(DAG\_FILE).nodes.log}.

  A few more notes:
  \begin{itemize}
  \item The expression \Expr{\$(LOG)} will not have the expected effect
  if used in the \MacroNI{DAGMAN\_DEFAULT\_NODE\_LOG} specification,
  because for \Condor{dagman} \Expr{\$(LOG)} is defined as \File{"."}.
  So if you want to put your default log file into the log directory
  you need to use something like \Expr{\$(LOCAL\_DIR)/log} instead of
  \Expr{\$(LOG)}.
  \item If you put your default log file into the spool directory,
  you will have to modify \MacroNI{VALID\_SPOOL\_FILES}
  (see ~\ref{param:ValidSpoolFiles}) to prevent \Condor{preen} from
  removing the default log file.  If the default log file is removed
  during a run, severe problems will ensue...
  \item The \Macro{DAGMAN\_DEFAULT\_NODE\_LOG} specification must
  ensure that the default node log files are named uniquely for each DAG.
  Therefore, the \Macro{DAGMAN\_DEFAULT\_NODE\_LOG} value should
  always include \File{@(DAG\_FILE)}.  For example,
  \begin{verbatim}
  ...
  \end{verbatim}
  will cause a failure if more than one DAG is run at a time on a
