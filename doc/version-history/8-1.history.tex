...
\item Several new python function calls were added to aid the composition of
ClassAd expressions.  It should now be possible to build expressions directly
in python without having to resort to string manipulation.  \Ticket{4154}
...
\item Setting \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} set to \Expr{False}
or using the corresponding \Opt{-dont\_use\_default\_node\_log} option
to \Condor{submit\_dag}
(in other words, having \Condor{dagman} read the log files specified
in the node job submit files) is no longer recommended.
See section~\ref{param:DAGmanAlwaysUseNodeLog} for more details.
\Ticket{4091}
...
\item Improve handling of the \Attr{coresize} submit file command.
In particular, allow values larger than 4GB.
\Ticket{4155}
